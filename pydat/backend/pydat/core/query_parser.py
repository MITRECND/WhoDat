...
class PyDatParser:
    tokens = PyDatLexer.tokens

    def p_query_group(self, t):
        'query : LPAREN query RPAREN'
        t[0] = t[2]

    def p_query_not(self, t):
        'query : NOT query'
    ...
    def create_combined_and(self, query1, query2):
        return {
            "query": {
                "bool": {
    ...
    def p_query_query(self, t):
        'query : query query %prec AND'
        t[0] = self.create_combined_and(t[1], t[2])

    def p_query_and_query(self, t):
        'query : query AND query'
        t[0] = self.create_combined_and(t[1], t[3])

    def p_query_or_query(self, t):
    ...
    def p_query_terminals(self, t):
        '''query : specific
                | daterange
                | termquery'''
        t[0] = t[1]

    def create_specific_word_subquery(self, key, value):
        fields1 = []
    ...
    def p_specific_fuzzy_word(self, t):
        'specific : FUZZY WORD COLON WORD'
        ...
        if len(t[1]) == 1:
            ...
            fuzzy = int(t[1][1])
        value = self.remove_escapes(value)

        sub_query = self.create_specific_word_subquery(key, value)
        sub_query['multi_match']['fuzziness'] = fuzzy

        t[0] = {'query': {'bool': {'must': [sub_query]}}}

    def p_specific_word(self, t):
    ...
    def p_specific_fuzzy_quoted(self, t):
        ...
        value = self.remove_escapes(value[1:-1])
        value = value.lower()
    ...
    def create_wildreg_query(self, key, value, qtype):
        fields1 = []
        fields2 = []
    ...
    def p_specific_wildcard(self, t):
        'specific : WORD COLON WILDCARD'

        key = t[1]
        value = t[3][2:-1]

        t[0] = self.create_wildreg_query(key, value, 'wildcard')

    def p_specific_regex(self, t):
        'specific : WORD COLON REGEX'

        key = t[1]
        value = t[3][2:-1]

        t[0] = self.create_wildreg_query(key, value, 'regexp')

    def create_daterange_query(self, key, start, end):
    ...
    def p_daterange_single(self, t):
        ...
        t[0] = self.create_daterange_query(t[1], start, end)
    ...
    def p_termquery_quoted(self, t):
        '''termquery : QUOTED'''

        term = self.remove_escapes(t[1][1:-1])
        term = term.lower()
        query = None
        terms = term.split()
        ...
        if ll is None:
            ...
            for (f, boost_val) in fields:
                if len(terms) > 1:
                    ...
                    shd = {'term': {
                        f: {"value": terms[0], 'boost': boost_val}}}
                    queries.append(shd)
    ...
    def p_termquery_word(self, t):
        '''termquery : WORD'''

        if len(t[1].split('.')) == 2:
            t[1] = f"'{t[1]}'"
            self.p_termquery_quoted(t)
            return

        term = self.remove_escapes(t[1])
        query = None
        ll = looks_like(term)
        if ll is None:
    ...
    def __init__(self, debug=False):
        self.lexer = PyDatLexer()
        self.parser = yacc.yacc(module=self, debug=debug)

    def parse(self, query, **kwargs):
        results = self.parser.parse(query, **kwargs)
        return results


parser = PyDatParser(debug=False)
