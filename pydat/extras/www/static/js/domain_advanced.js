$(document).ready(function() {
    ...
    var ajax_url = advDataTable_url + "?query=" + query_string + "&csrfmiddlewaretoken=" + csrf_token;
...
*/
                    ...
                    "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                        ...
                        $('td:eq(7)', nRow).addClass(oldVersion);
...
function process_query(query){
    var ajax_url = query_url + "?query=" + query + "&csrfmiddlewaretoken=" + csrf_token;
    $.ajax({
        type: 'GET',
        url: ajax_url,
        datatype: 'json',
        ...
        success: function(response){
            if (response.success) {
                var start = response['skip'] + 1;
                var stop = start + response['avail'] - 1;
                var total = response['total'];
                $("#results_header").html("<p> Displaying " + start + " - " + stop + " of " + total +"</p>");
                for (var i = 0; i < response.avail; i++){
                    res = document.createElement('div');
                    $(res).addClass('result');
                    $(res).html("<p>" + response.data[i].domainName + "</p>");
                    $("#results").append(res);
                }
            }
        }
